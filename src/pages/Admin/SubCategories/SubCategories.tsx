    ...
    const fetchSubCategories = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/sub-categorias`,
          {
            headers: {
  ...
  return (
    <MainContainer>
      ...
      <div className='mt-6 w-12xl'>
        <button
          onClick={() => navigate("/administrar/sub-categorias/nuevo")}
        ...
        <div className='mt-4'>
          <Table
            ...
            headers={["ID", "Categoría", "Nombre", "Acciones"]}
            itemsPerPage={5}
            renderRow={(row: any) => (
              <>
                ...
                <td className='px-6 py-4 whitespace-nowrap text-sm font-medium'>
                  <button
                    onClick={() =>
                      navigate(`/administrar/sub-categorias/${row.id}`)
                    }
                    ...
                    onClick={() => {
                      Swal.fire({
                        ...
                        if (res.isConfirmed) {
                          axios
                            .delete(
                              `${import.meta.env.VITE_API_URL}/sub-categorias/${row.id}`,
                            )
                            .then(() => {
