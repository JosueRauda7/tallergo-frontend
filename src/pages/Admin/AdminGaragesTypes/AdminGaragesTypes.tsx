...
const AdminGaragesTypes = () => {
  ...
  useEffect(() => {
    const fetchGaragesTypes = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/tipo-talleres`);
        setRows(res.data);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <MainContainer>
      <div className="">
        <h1 className="text-4xl font-bold">Administración de Tipos de Talleres</h1>
      </div>
      <div className="mt-6 w-12xl">
        <button onClick={() => navigate('/administrar/tipos-talleres/nuevo')} className="bg-blue-700 text-white px-4 py-2 rounded mt-2 cursor-pointer hover:bg-blue-800">
          Agregar Tipo de Taller
        </button>
        <div className="mt-4">
          <Table
...
export default AdminGaragesTypes;