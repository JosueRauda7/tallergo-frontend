  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (id) {
        ...
        setInitialValues({
          tipoUsuarioId: data.tipoUsuario.id,
    ...
    const fetchUserTypes = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/tipo-usuarios`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      const data = await res.json();
      const tipoUsuarios = data.map((type: TipoUsuariosResponse) => ({
        id: type.id,
        nombre: type.nombre,
      }));
      console.log(tipoUsuarios);
      setUserTypes(tipoUsuarios);
    };
