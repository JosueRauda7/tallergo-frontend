...
const Home = () => {
  ...
  useEffect(() => {
    if (latitude === null || longitude === null) {
      ...
      const talleresRes = async () => {
        const talleresData = await axios.get(
          `${import.meta.env.VITE_API_URL}/talleres?&distancia=10&latitud=${latitude}&longitud=${longitude}`,
          {
            headers: {
      ...
      <div className='mt-6 py-6 bg-gray-200 rounded-2xl shadow-lg shadow-gray-400'>
        ...
        <div className='mt-4 p-2'>
          <div className='px-4'>
            ...
            <p className='text-lg'>
              {markerSelected
                ? markerSelected
                : "Seleccione un taller en el mapa."}
            </p>
          </div>
        ...
        <div className='mt-4 px-4'>
          <Maps
            ...
            height='70vh'
            marker
            enableChangeMarkerPosition={false}
            mapMarkers={talleres.map((taller: any) => ({
              latitude: taller.latitud,
              longitude: taller.longitud,
              title: `
                <p class="text-lg mb-1"><b>${taller.nombre}</b></p>
                <p class="text-sm mb-1">${taller.tipoTaller.nombre}</p>
                <p class="text-xs">Distancia: ${taller.distancia.toFixed(2)} km</p>
              `,
              action: () => {
                setLatitudeToGo(taller.latitud);
                setLongitudeToGo(taller.longitud);
                setMarkerSelected(taller.nombre);
              },
            }))}
          />
        </div>
